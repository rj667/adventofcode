...
start_register = register.copy()
for A in range(start_register['A'], 0, -1):
    register = {'A': A, 'B': start_register['B'], 'C': start_register['C']}
    pointer = 0
    output = []
    ...
    if ','.join(str(_) for _ in output) == ','.join(str(_) for _ in program):
        break
